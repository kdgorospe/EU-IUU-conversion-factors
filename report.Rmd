...
![](Images/Section - CF ranges and data coverage/CF_EU_values_yes_no.png)
...
Summing across presentations and states, the total estimated Atlantic cod can be estimated using Member State versus EU-wide values (Figure X) and across the entire EU (Figure X). In this case, the application of the country-level values for Portugal and Norway result in higher estimated catch than the application of the EU-wide values (Figures X and Y). Notably, although the relative difference between the estimates may appear small, when applied to a large quantity, it can result in a large difference in estimated tonnes caught (Figure X). 
...
While in the Atlantic cod case Norway and Portugal's estimated catch fell above the estimate using the EU-wide value, using the national values for each country results in an estimated catch below the catch estimated with the EU value (Figure X). Summing across all countries and presentations and states reveals the differences in the total hake catch (Figure X). For hake, there are several points where there are substantial volumes of product landed in presentations and states without an available conversion factor (Figure X). This highlights an important data gap issue for calculating hake catch.  
