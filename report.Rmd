...
Hake (*Merluccius merluccius*) is another historically important fishery in Europe. 
* Even though there is a wide range of values for hake, the countries landing hake don't differ much in their CF values from the EU
* Whereas above, the Norway and Portugal's estimated catch fell above the EU, their estimate's fall below the EU value (when summing across presentations and states)
Yet depending on the species we see different patterns with regards to how nominal catch compares to the EU-wide conversion factor. We show two cases (Portugal and Norway) where for one species applying the national-level value results in a higher nominal catch, while for the other species applying the EU-wide level value results in a higher nominal catch. 

* Large spikes of landings without a CF (often listed as "all presentations")
...
* Like above, even though there is a wide range of values for hake, the countries landing hake don't differ much in their CF values from the EU 
...
* Even those that are catching it, don't catch that much of it, so you don't get that big of a range in absolute differences
* The difference between min and max is smaller than might be expected due to the data gaps (e.g., cases where there is only one value available)
...
European Commission. n.d. Cod (Gadus morhua). <https://ec.europa.eu/fisheries/marine_species/wild_species/cod_en>
