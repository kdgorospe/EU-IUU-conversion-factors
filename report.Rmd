...
![Illustration of estimated live weight equivalent of catch for different presentations and ranges of live weight conversion factors](Images/LWE_calc_fig.png){width=75%}
...
When considering just the official conversion factors implemented by the European Commission's Regulation (EU) No 404/2011 for EU-wide and Member State fisheries products, and merge this with the top 1600 species and presentations, which accounts for 87.1 % of total landings from 1992 - 2018 landed by fishing vessels from the 27 EU countries, we see considerable data gaps that hinder our (and the EU's) ability to calculate nominal match for these landed product (Figure 3). A total of 971 (60.7%) of the 1600 species and presentations have no CF value whatsoever (i.e., neither a national or EU-wide value). Part of this stems from the fact that the presentations listed by the European Commission do not properly align with those found in Eurostat's landings data. 

![Grid showing the top 50 species by landed weight in the EU (accounts for 87.1 % of total landings from 1992 - 2018). Gray areas are species, states, and presentations that are not landed. Shaded blue areas represent the proportion of all countries that reportedly landed a species, state, and presentation (i.e., product) that also have a national-level conversion factor for that product.](Images/Section - CF ranges and data coverage/CF_natonal_values_heat_map.png)

![Grid showing the top 50 species by landed weight in the EU (accounts for 87.1 % of total landings from 1992 - 2018). Gray areas are species, states, and presentations that are not landed. Green areas represent products that have an EU-wide level conversior factor. Pink areas represent products that have no EU-wide level conversion factor.](Images/Section - CF ranges and data coverage/CF_EU_values_yes_no.png)

## EU Fishery Case Studies

Below are several case studies focusing on three species of economic importance to the EU: cod (*Gadus morhua*), hake (*Merluccius merluccius*), and Monkfish (*Lophiidae*). For this analysis, we include the conversion factors from Norway's National Fisheries Authority. Although Norway is not part of the EU, they do follow certain fisheries regulations outlined by the European Commission. For the same reason, we attempted to include the national conversion factors of Greenland and the Faroe Islands, but were unable to reliably match these to the landings data for the case study species (they are only reported as "All presentation forms"). For the landings data in this analysis, we include vessels of all nationalities, regardless of EU membership.

While the previous section showed serious data gaps in conversion factors, this section highlights focuses on how differences in live weight conversion factors translate into differences in estimated catch. 

For each species case study, we present a bar graph representing a snapshot of landings data for each country as well as the calculation of nominal catch when the fishing vessel's national conversion factor and/or the EU-wide conversion factor is applied. We present the product presentations with the largest relative ranges in conversion factors. In the case of Norway, we see that one problem that arises when expanding our analysis to include "third country" landings and conversion factors is that the presentation forms do not perfectly align. For example, for fresh, gutted, and headed Atlantic cod (*Gadus morhua*), Norway has three different conversion factors that would fall under this category (gutted and headed with either a round cut, right cut, or earbone removed). For fresh, gutted, and headed hake, (*Merluccius merluccius*), Norway has two different conversion factors that could potentially be applied. 

We then present time series of estimated catch, first for individual countries and then for total landings. In national plots, we illustrate the differences in estimated catch with the Member State versus the EU conversion factors while in the Eu-wide plots we illustrate the difference in estimated catch when applying the minimum versus the maximum conversion factor for each presentation and state. 

### Atlantic cod (*Gadus morhua*)

Atlantic cod (*Gadus morhua*) is historically amoung the most important fisheries in Europe. The Atlantic cod stocks throughout Europe have declined in recent decades, in part due to overfishing (European Commission, n.d. a). As a result, reliable catch data is particularly important for managing these stocks. Several presentation and state combinations for Atlantic cod also appeared toward the top of the ranked ranges in conversion factors (both in abosulte and relative terms; Figures XX and XX). The range in live weight conversion factors is evident when calculating the nominal catch from landings for each country (Figure XX). 

Within the EU, there is variability in the estimated catch based on the Member State versus the EU-wide conversion factor (Figure Xa). The largest difference for fresh, gutted and headed Atlantic cod occurs when comparing France's conversion factor to the EU-wide value (Figure Xa). The total volumes of catch in this example are relatively small though, especially when compared to the volumes landed by Norway (Figure Xb). Under the catch certificate regulation, Norway, as a non-Member State, would not be required to use the EU value. Norway provides three different live weight conversion factors for fresh, gutted and headed Atlantic cod (gutted and headed with right cut, round cut and with earbone removed). Depending on which of these is applied, the estimated catch may be lower than the estimated catch using the EU-wide value (Figure Xb). 

![Landings (grey) and estimated catch using national (orange) and the EU-wide (blue) conversion factor values for fresh, gutted and headed Atlantic cod. Note for Norway, there are three conversion factors that could potentially be applied (gutted and headed with round cut, right cut, or with earbone removed)](Images/Section - EU Fishery Case Studies/Proposed Final Plots/Bar Graphs/cod_snapshot_country_vs_EU_multipanel.png)

Summing across presentations and states, the total estimated Atlantic cod can be estimated using Member State versus EU-wide values (Figure X) and across the entire EU (Figure X). In this case, the application of the country-level values for Portugal and Norway result in lower estimated catch than the application of the EU-wide values (Figure X). Notably, although the relative difference between the estimates may appear small, when applied to a large quantity, it can result in a large difference in estimated tonnes caught (Figure X). 

As seen in the time series (Figure ), when comparing the application of the national versus EU-wide conversion factor values, the largest gap in estimated catch for Portugal was 2096 tonnes (in 2011) and for Norway was 13835 tonnes (in 2014). When summing across the entire EU and comparing the minimum vs maximum available CF values across all countries, the largest discrepancy for estimated catch of cod was 12273 tonnes (in 2015). 

![Time series of Atlantic cod tonnes landed by product form (colored lines) and estimated catch (black lines) using the Member State (dotted) versus the EU (dashed) conversion factor for Portugal (A) and Norway (B)](Images/Section - EU Fishery Case Studies/Proposed Final Plots/Time Series Country vs EU values/cod_country_vs_EU_multipanel.png)

![Time series of all Atlantic cod landings and estmated catch in the EU, based on the minimum and maximum conversion factor (black lines)](Images/Section - EU Fishery Case Studies/Proposed Final Plots/Time Series of min vs max CF/landings-vs-catch_min-vs-max-CF_Gadus-morhua.png)

### Hake (*Merluccius merluccius*)

Hake (*Merluccius merluccius*) is another historically important fishery in Europe. Following concerns about the sustainability of the hake stocks, recovery plans were implemented in 2004 and 2005 for the Northern and Southern hake, respectively (European Commision, n.d. b), which includes catch limits. Some presentations and states for hake include a wide range of conversion factor values. Much of this variation is contributed by countries that do not land hake in these presentations and states. As a result, the estimated catch using different countries' conversion factors results in fairly limited differences compared to the EU-wide value (Figure X). 

![Landings (grey) and estimated catch using national (orange) and the EU-wide (blue) conversion factor values for fresh, gutted and headed hake](Images/Section - EU Fishery Case Studies/Proposed Final Plots/Bar Graphs/landings_vs_catch_case_study_10_Merluccius merluccius Fresh, gutted and headed.png)

While in the Atlantic cod case Norway and Portugal's estimated catch fell below the estimate using the EU-wide value, using the national values for each country results in an estimated hake catch above the catch estimated with the EU value (Figure X). Summing across all countries and presentations and states reveals the differences in the total hake catch (Figure X). For hake, there are several points where there are substantial volumes of product landed in presentations and states without an available conversion factor (Figure X). This highlights an important data gap issue for calculating hake catch.  

The largest gap in estimated catch based on comparing the national vs EU-wide values for Portugal was 2.47 tonnes (in 2009) and for Norway was 305 tonnes (in 2016). When summing across the entire EU and comparing the minimum vs maximum available CF values across all countries, the largest discrepancy in estimated catch for hake was 6461 tonnes (in 2015). 

![Time series of Hake tonnes landed by product form (colored lines) and estimated catch (black lines) using the Member State (dotted) versus the EU (dashed) conversion factor for Portugal (A) and Norway (B)](Images/Section - EU Fishery Case Studies/Proposed Final Plots/Time Series Country vs EU values/hake_country_vs_EU_multipanel.png)

![Time series of all hake landings and estmated catch in the EU, based on the minimum and maximum conversion factor (black lines)](Images/Section - EU Fishery Case Studies/Proposed Final Plots/Time Series of min vs max CF/landings-vs-catch_min-vs-max-CF_Merluccius-merluccius.png)

### Monkfish (*Lophiidae*)

Monkfish rose to the top of the ranking of reported live weight conversion factor ranges, with a maximum conversion factor for frozen, gutted, and headed over 2.5 times the minimum conversion factor (Figure X). Similar to the hake case, however, despite this wide range of conversion factors, the countries landing monkfish do not have established conversion factors that differ substantially from the EU values (Figure X). The minimum value for frozen, gutted, and headed *Lophiidae* was reported by Portugal, but this product was not landed in recent years by any Portuguese fishing vessels, which is why there is a narrower range in calculated nominal catch than would be expected based on the range of all conversion factors (Figure X). Additionally, there are several countries that land fresh, gutted, and headed *Lophiidae* in the EU, but of these, only Sweden has a national value despite the fact that Belgium lands this product in quantities two orders of magnitude greater than the rest of the countries. Further, in all cases, the difference between the minimum value and maximum value is smaller than might be expected due to the data gaps that result in a single value for a given presentation and state. 

When summing across the entire EU and comparing the minimum vs maximum available CF values across all countries, the largest discrepancy in estimated catch for monkfish was 418 tonnes (in 2017). 


![Landings (grey) and estimated catch using national (orange) and the EU-wide (blue) conversion factor values for fresh, gutted and headed monkfish](Images/Section - EU Fishery Case Studies/Proposed Final Plots/Bar Graphs/monkfish_snapshot_country_vs_EU_multipanel.png)


![Time series of all monkfish landings and estmated catch in the EU, based on the minimum and maximum conversion factor (black lines)](Images/Section - EU Fishery Case Studies/Proposed Final Plots/Time Series of min vs max CF/landings-vs-catch_min-vs-max-CF_Lophiidae.png)
...
European Commission. n.d. a Cod (Gadus morhua). <https://ec.europa.eu/fisheries/marine_species/wild_species/cod_en>.

European Commission. n.d. b Multi-annual plans currently in place. <https://ec.europa.eu/fisheries/cfp/fishing_rules/multi_annual_plans/list_of_plans_en>.
